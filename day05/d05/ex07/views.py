...
def display(requests):
    try:
        ...
        return render(requests, "ex07/display.html", {'data': l})
    except Exception as e:
...
def update(req):
    ...
    try:
        ...
        if (req.method == 'POST'):
            for x in res:
                if (x.title == req.POST.get('title')):
                    id = x.episode_nb
                    break
            if (id != None):
                tmpR = Movies.objects.get(episode_nb=id)
                tmpR.opening_crawl = data
                tmpR.save()
        res = Movies.objects.all()
        for x in res:
            l.append(x.title)
        return render(req, "ex07/update.html", {"form": form, "data": l})
    except Exception as e:
         return  (HttpResponse("No data available"))